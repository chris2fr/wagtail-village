...
from sites_faciles.managers import TagManager
from sites_faciles.sites_faciles_blog.blocks import COLOPHON_BLOCKS
from sites_faciles.sites_faciles_blog.managers import CategoryManager
from sites_faciles.sites_faciles_forms.widgets import SitesFacilesRadioSelect
from sites_faciles.sites_faciles_page.models import ContentPage, SitesFacilesBasePage, Tag, WagtailSitesFacilesConfig
from sites_faciles.widgets import WagtailSitesFacilesIconPickerWidget
...
class BlogIndexPage(ContentPage):
    ...
    subpage_types = ["sites_faciles.BlogEntryPage", "sites_faciles.DirectoryEntryPage"]
        ...
        if tag:
            ...
            breadcrumb = {
                "links": [
                    ...
                    {
                        "url": reverse("sites_faciles_blog:tags_list", kwargs={"blog_slug": self.slug}),
                        "title": _("Tags"),
                    },
                ],
                "current": tag,
        ...
        if category:
            ...
            breadcrumb = {
                "links": [
                    {"url": self.get_url(), "title": self.title},
                    {
                        "url": reverse("sites_faciles_blog:categories_list", kwargs={"blog_slug": self.slug}),
                        "title": _("Categories"),
                    },
                ],
...
class DirectoryIndexPage(BlogIndexPage):
    ...
    subpage_types = ["sites_faciles.DirectoryEntryPage"]
...
class BlogEntryPage(ContentPage):
    ...
    authors = ParentalManyToManyField(
        "sites_faciles.Person", blank=True, help_text=_("Author entries can be created in Snippets > Persons")
    )

    parent_page_types = ["sites_faciles.BlogIndexPage", "sites_faciles.DirectoryIndexPage"]
    subpage_types = []
...
class DirectoryEntryPage(BlogEntryPage):
    ...
    parent_page_types = ["sites_faciles.DirectoryIndexPage"]
