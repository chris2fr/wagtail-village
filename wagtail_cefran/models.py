...
from wagtail_cefran.abstract import SitesFacilesBasePage, WagtailCefranConfig
from wagtail_cefran.managers import TagManager
...
class SocialMediaItem(Orderable):
    site_config = ParentalKey(WagtailCefranConfig, related_name="social_media_items")
    ...
    panels = [
        FieldPanel("title"),
        FieldPanel("url"),
        FieldPanel("icon_class", widget=WagtailCefranIconPickerWidget),
    ]
...
class MegaMenuCategory(Orderable):
    mega_menu = ParentalKey("wagtail_cefran.MegaMenu", related_name="categories", on_delete=models.CASCADE)
