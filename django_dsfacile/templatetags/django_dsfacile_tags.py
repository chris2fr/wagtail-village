...
from django_dsfacile.utils import (
    django_dsfacile_input_class_attr,
...
"""
Tags used in the "DSFACILE" templates.
"""

# Global tags


@register.inclusion_tag("django_dsfacile/global_css.html")
def dsfacile_css() -> dict:
    """
    Returns the HTML for the CSS header tags for DSFACILE
...
@register.inclusion_tag("django_dsfacile/global_js.html", takes_context=True)
def dsfacile_js(context, *args, **kwargs) -> dict:
    """
    Returns the HTML for the JS body tags for DSFACILE
...
@register.inclusion_tag("django_dsfacile/favicon.html")
def dsfacile_favicon() -> dict:
    """
    Returns the HTML for the CSS header tags for the DSFACILE "Marianne" Favicon
...
@register.inclusion_tag("django_dsfacile/theme_modale.html")
def dsfacile_theme_modale() -> None:
    """
    Returns the HTML for the theme selection modale for DSFACILE

    **Tag name**:
        dsfacile_theme_modale
...
@register.inclusion_tag("django_dsfacile/accordion.html")
def dsfacile_accordion(*args, **kwargs) -> dict:
    """
    ...
    Can be used alone or in a group with the tag `dsfacile_accordion_group`.

    **Tag name**:
        dsfacile_accordion

    **Usage**:
        `{% dsfacile_accordion data_dict %}`
    """
...
@register.inclusion_tag("django_dsfacile/accordion_group.html")
def dsfacile_accordion_group(items: list) -> dict:
    """
    ...
    **Tag name**:
        dsfacile_accordion_group

    **Usage**:
        `{% dsfacile_accordion_group data_list %}`
    """
    return {"self": {"items": items}}


@register.inclusion_tag("django_dsfacile/alert.html")
def dsfacile_alert(*args, **kwargs) -> dict:
    """
...
@register.inclusion_tag("django_dsfacile/badge.html")
def dsfacile_badge(*args, **kwargs) -> dict:
    """
    ...
    - `dsfacile-badge--sm`: small-sized badge
    - `dsfacile-badge--green-menthe` other color codes: change the color of the badge
    - `dsfacile-badge--success` (or error/info/warning/new): system badges
    - `dsfacile-badge--no-icon`: removes the icon on system badges

    **Tag name**:
        dsfacile_badge

    **Usage**:
        `{% dsfacile_badge data_dict %}`
    """
    allowed_keys = [
        "label",
        "extra_classes",
...
@register.inclusion_tag("django_dsfacile/badge_group.html")
def dsfacile_badge_group(items: list) -> dict:
    """
    ...
    **Tag name**:
        dsfacile_badge_group

    **Usage**:
        `{% dsfacile_badge_group data_list %}`
    """
    return {"self": {"items": items}}


@register.inclusion_tag("django_dsfacile/breadcrumb.html", takes_context=True)
def dsfacile_breadcrumb(context: Context, tag_data: dict = {}) -> dict:
    """
...
@register.inclusion_tag("django_dsfacile/button.html")
def dsfacile_button(*args, **kwargs) -> dict:
    """
    ...
    - `dsfacile-btn--secondary`: secundary button
    - `dsfacile-btn--tertiary`: tertiary button
    - `dsfacile-btn--tertiary-no-outline`: tertiary button with no outline
    - `dsfacile-btn--icon-left` and `dsfacile-btn--icon-right`: add an icon to the button
      (associated with an icon class)
    - `dsfacile-btn--sm` and `dsfacile-btn--lg`: button smaller or larger than the default size

    **Tag name**:
        dsfacile_button

    **Usage**:
        `{% dsfacile_button data_dict %}`
    """
    allowed_keys = [
        "label",
        "name",
...
@register.inclusion_tag("django_dsfacile/button_group.html")
def dsfacile_button_group(*args, **kwargs) -> dict:
    """
    ...
    - `dsfacile-btns-group--inline-sm`: Inline group, small size
    - `dsfacile-btns-group--inline-md`: Inline group, normal size
    - `dsfacile-btns-group--inline-lg`: Inline group, large size
    - `dsfacile-btns-group--sm`: Vertical group, small size
    - `dsfacile-btns-group--lg`: Vertical group, large size
    - `dsfacile-btns-group--equisized`: Width adjusted in Javascript
    - `dsfacile-btns-group--icon-left`: Buttons with an icon on the left side
    - `dsfacile-btns-group--icon-right`: Buttons with an icon on the right side

    **Tag name**:
        dsfacile_button_group

    **Usage**:
        `{% dsfacile_button_group data_dict %}`
    """
    allowed_keys = [
        "items",
        "extra_classes",
...
@register.inclusion_tag("django_dsfacile/callout.html")
def dsfacile_callout(*args, **kwargs) -> dict:
    """
    ...
    - Color classes ([See the list](/django-dsfacile/resources/colors)), for example `dsfacile-callout--green-emeraude`

    **Tag name**:
        dsfacile_callout

    **Usage**:
        `{% dsfacile_callout data_dict %}`
    """
    allowed_keys = [
        "text",
        "title",
...
@register.inclusion_tag("django_dsfacile/card.html")
def dsfacile_card(*args, **kwargs) -> dict:
    """
    ...
    - `dsfacile-card--horizontal`: makes the card horizontal
    - `dsfacile-card--horizontal-tier`: allows a 33% ratio instead of the 40% default
    - `dsfacile-card--horizontal-half`: allows a 50% ratio instead of the 40% default
    - `dsfacile-card--download`: replaces the forward arrow icon with a download one
    - `dsfacile-card--grey`: adds a grey background on the card
    - `dsfacile-card--no-border`: removes the card border
    - `dsfacile-card--no-background`: removes the card background
    - `dsfacile-card--shadow`: adds a shadow to the card border
    ...
    top_detail = {
        "detail": {
            ...
            "icon_class": "(Optional) an icon class (eg, dsfacile-icon-warning-fill)"
        },
...
@register.inclusion_tag("django_dsfacile/consent.html")
def dsfacile_consent(*args, **kwargs) -> dict:
    """
...
@register.inclusion_tag("django_dsfacile/content.html")
def dsfacile_content(*args, **kwargs) -> dict:
    """
    ...
    data_dict = {
        ...
        "transcription": "(Optional) A transcription item dictionary,
                         see [component documentation](/django-dsfacile/components/transcription/)",
    }
    ```
    ...
    - `dsfacile-content-media--lg`: media is 125% of the main text width.
    - `dsfacile-content-media--sm`: media is 75% of the the main text width.
...
@register.inclusion_tag("django_dsfacile/france_connect.html")
def dsfacile_france_connect(*args, **kwargs) -> dict:
    """
    ...
    **Tag name**:
        dsfacile_france_connect

    **Usage**:
        `{% dsfacile_france_connect data_dict %}`
    """
...
@register.inclusion_tag("django_dsfacile/highlight.html")
def dsfacile_highlight(*args, **kwargs) -> dict:
    """
    ...
    - Color classes ([See the list](/django-dsfacile/resources/colors)),
    for example `dsfacile-highlight--green-emeraude`
...
@register.inclusion_tag("django_dsfacile/input.html")
def dsfacile_input(*args, **kwargs) -> dict:
    """
...
@register.inclusion_tag("django_dsfacile/link.html")
def dsfacile_link(*args, **kwargs) -> dict:
    """
    ...
    - `dsfacile-link--icon-left` or `dsfacile-link--icon-right` with an icon class
    - `dsfacile-link--sm` for small links
    - `dsfacile-link--lg` for large links
...
@register.inclusion_tag("django_dsfacile/notice.html")
def dsfacile_notice(*args, **kwargs) -> dict:
    """
...
@register.inclusion_tag("django_dsfacile/pagination.html", takes_context=True)
def dsfacile_pagination(context: Context, page_obj: Page) -> dict:
    """
    ...
    **Usage**:
        `{% dsfacile_pagination page_obj %}`
    """
    return {"request": context["request"], "page_obj": page_obj}


@register.inclusion_tag("django_dsfacile/quote.html")
def dsfacile_quote(*args, **kwargs) -> dict:
    """
    ...
    - Color classes ([See the list](/django-dsfacile/resources/colors)), for example `dsfacile-quote--green-emeraude`

    **Tag name**:
        dsfacile_quote

    **Usage**:
        `{% dsfacile_quote data_dict %}`
    """
...
@register.inclusion_tag("django_dsfacile/select.html")
def dsfacile_select(*args, **kwargs) -> dict:
    """
    ...
    - `dsfacile-sidemenu--sticky`: Makes the menu sticky
    - `dsfacile-sidemenu--sticky-full-height`: Makes the menu take the full height of the screen
    - `dsfacile-sidemenu--right`: Moves the menu to the right side of the screen
...
@register.inclusion_tag("django_dsfacile/sidemenu.html", takes_context=True)
def dsfacile_sidemenu(context: Context, *args, **kwargs) -> dict:
    """
...
@register.inclusion_tag("django_dsfacile/skiplinks.html", takes_context=True)
def dsfacile_skiplinks(context: Context, items: list) -> dict:
    """
...
@register.inclusion_tag("django_dsfacile/stepper.html")
def dsfacile_stepper(*args, **kwargs) -> dict:
    """
...
@register.inclusion_tag("django_dsfacile/summary.html")
def dsfacile_summary(items: list, heading_tag: str = "p") -> dict:
    """
    ...
    **Usage**:
        `{% dsfacile_summary items heading_tag %}`
    """
    return {"self": {"items": items, "heading_tag": heading_tag}}


@register.inclusion_tag("django_dsfacile/table.html")
def dsfacile_table(*args, **kwargs) -> dict:
    """
    ...
    - Color classes ([See the list](/django-dsfacile/resources/colors)), for example `dsfacile-table--green-emeraude`
    - `dsfacile-table--bordered`: adds a border under each line
    - `dsfacile-table--no-scroll` prevents horizontal scrolling on mobile
    - `dsfacile-table--layout-fixed`: forces the table at 100% and equal size columns
    - `dsfacile-table--no-caption`: hides the caption
    - `dsfacile-table--caption-bottom`: sets the caption after the table instead of before

    **Tag name**:
        dsfacile_table

    **Usage**:
        `{% dsfacile_table data_dict %}`
    """
    allowed_keys = [
        "caption",
        "content",
...
@register.inclusion_tag("django_dsfacile/tag.html")
def dsfacile_tag(*args, **kwargs) -> dict:
    """
    ...
    - `dsfacile-tag--sm`: for a small tag
    - icon classes: an icon for the tag, along with a positional class
      (eg, `dsfacile-icon-arrow-right-line` `dsfacile-tag--icon-left`)
...
@register.inclusion_tag("django_dsfacile/tile.html")
def dsfacile_tile(*args, **kwargs) -> dict:
    """
    ...
    - `dsfacile-tile--sm`: for a small (SM) tile
    - `dsfacile-tile--horizontal`: for an horizontal tile
    - `dsfacile-tile--download`: Replaces the forward arrow icon with a download one
    - `dsfacile-tile--grey`: adds a grey background on the tile
    - `dsfacile-tile--no-border`: removes the tile border
    - `dsfacile-tile--no-background`: removes the tile background
    - `dsfacile-tile--shadow`: adds a shadow to the tile border
...
@register.inclusion_tag("django_dsfacile/toggle.html")
def dsfacile_toggle(*args, **kwargs) -> dict:
    """
    ...
    - `dsfacile-toggle--label-left`: sets the label on the left side
    - `dsfacile-toggle--border-bottom`: adds a border at the bottom
...
@register.inclusion_tag("django_dsfacile/tooltip.html")
def dsfacile_tooltip(*args, **kwargs) -> dict:
    """
...
@register.inclusion_tag("django_dsfacile/transcription.html")
def dsfacile_transcription(*args, **kwargs) -> dict:
    """
    ...
    **Tag name**:
        dsfacile_transcription

    **Usage**:
        `{% dsfacile_transcription data_dict %}`
    """
...
def dsfacile_django_messages(context, is_collapsible=False, extra_classes=None, wrapper_classes=None):
    """
    Renders django messages in a series a DSFACILE alerts

    ```python
    data_dict = {
        "is_collapsible" : "(Optional) Boolean, set to true to add a 'close' button for the alert (default: false)",
        "wrapper_classes": "(Optional) extra classes for the wrapper of the alerts (default `dsfacile-my-4v`)",
    ...
    <div class="dsfacile-table" markdown="1">
    ...
    There types are then concatenated with ``dsfacile-alert--`` to form the CSS classe in the template.

    These classes can be modified by setting ``DSFACILE_MESSAGE_TAGS_CSS_CLASSES`` in your ``settings.py``, like so:

    ```python
    from django.contrib import messages
    DSFACILE_MESSAGE_TAGS_CSS_CLASSES = {
    ...
    renders an alert with the following CSS class: `dsfacile-alert--warning`.

    **Tag name**:
        dsfacile_django_messages

    **Usage**:
        `{% dsfacile_django_messages data_dict %}`
    """  # noqa
    ...
    wrapper_classes = wrapper_classes or "dsfacile-my-4v"
    ...
    message_tags_css_classes = {
        ...
        WARNING: "warning",
        ERROR: "error",
        **getattr(settings, "DSFACILE_MESSAGE_TAGS_CSS_CLASSES", {}),
    }

    def _render_alert_tag(message):
        return Template("{% load django_dsfacile_tags %}{% dsfacile_alert data_dict %}").render(
            Context(
                {
                    "data_dict": {
...
@register.inclusion_tag("django_dsfacile/form_snippet.html", takes_context=True)
def dsfacile_form(context) -> dict:
    """
...
@register.inclusion_tag("django_dsfacile/form_field_snippets/field_snippet.html")
def dsfacile_form_field(field) -> dict:
    """
    ...
    **Usage**:
        `{% dsfacile_form_field field %}`
    """
    return {"field": field}


register.filter(name="django_dsfacile_input_class_attr", filter_func=django_dsfacile_input_class_attr)
